# 欢迎来到 Among Us Revamped！（汉化版）by 芙酱awa和這是名字awa

# We were only responsible for translating all the code in this project; we did not plagiarize. We hope the author will be understanding, as we have already indicated the source.

<p align="center">

<center>
<a href="https://discord.gg/83Zhzhyhya" target="_blank">
<img src="https://img.shields.io/badge/Discord%20-%231DA1F2.svg?&style=for-the-badge&logo=discord&logoColor=white&color=5662f6"/>
</a>

<a href="https://github.com/apemv/AmongUsRevamped/releases/latest" target="_blank">
<img src="https://img.shields.io/badge/Latest Version%20-%231DA1F2.svg?&style=for-the-badge&logo=github&logoColor=white&color=181717"/>
</a>

<a href="https://b23.tv/r3e1u0x" target="_blank">
<img src="https://i.pinimg.com/736x/ba/89/80/ba89800eb4ab6110c8283c79a45a917c.jpg"/>
<h6>這是名字awa </h6> 

<a href="https://b23.tv/h3Hnqqf" target="_blank">
<img src="https://i.pinimg.com/736x/ba/89/80/ba89800eb4ab6110c8283c79a45a917c.jpg"/>
<h6>芙酱awa </h6> 
  
</center>

</p>

---

# 该作者的链接在上方请关注他

---

# 💡 安装方法：

- **如果你已经安装 BepInEx：**  
  下载 Releases 里的 AUR.dll，并放入：  
  `Among Us → BepInEx → Plugins`

- **如果你没有安装 BepInEx：**
  - 下载 Releases 页面最新 `.zip`（已发布）
  - 解压到 Among Us 游戏目录
  - 直接覆盖文件

- 游戏目录位置：
  - **Steam：** 库 → 右键 Among Us → 管理 → 浏览本地文件  
  - **Epic：** 库 → 管理 → 文件夹图标  
  - **Itch.io：** 管理 → 打开文件夹  
  - **Xbox App：** 管理 → 文件 → 浏览  
  - **Microsoft Store：** 见官方说明  
    https://answers.microsoft.com/en-us/xbox/forum/all/where-can-i-find-the-gamefiles-of-a-game/5cb9a0c3-7948-4316-abc5-f27d1767b932  

---

# 🎮 功能（v1.7.0）

## 🌟 自定义角色
- 市长（船员）
- 小丑（中立)

## ♠️ 客户端设置
- 游戏管理员
- 解锁 FPS / 显示 FPS
- 黑暗主题
- 大厅音乐开关
- 可自定义文本

## ⚙️ 技术设置
- 踢/封低等级玩家
- 踢/封无效 好友代码
- 踢说 “开或开始" 的玩家
- 禁词系统 / 黑名单
- 多层管理员系统
- AUR-DATA 添加管理员/高级会员（VIP）
- 不结束游戏模式
- 自定义开局倒计时

## ♻️ 自动化
- 自动开局
- 自动重进房间
- 自动发送胜利信息

## 🔧 游戏玩法
- 死亡内鬼可破坏
- 关闭各种破坏
- 禁止开局30秒开会
- 禁止报告尸体
- 地图功能禁用
- 自定义任务系统
- 捉迷藏模式优化

## 🏆 自定义模式
- 0冷却击杀
- 变形与躲猫猫
- 速通

## ❓ 其他优化
- 反转 Skeld 地图
- 可取消倒计时
- 更快启动
- 自定义反作弊
- 大厅/死亡缩放
- 死亡可看统计
- 高级选项范围
- 支持公开房间

---

# ⌨️ 快捷键

| 快捷键 | 功能 | 场景 |
|--------|------|------|
| Shift+L+Enter | 强制结束游戏 | 游戏中 |
| Shift+M+Enter | 跳过会议 | 游戏中 |
| C | 取消开局 | 倒计时 |
| Shift | 立即开局 / ×5选项 | 倒计时 / 设置 |
| Ctrl | Noclip / ×10选项 | 大厅 / 设置 |

---

# 💬 聊天指令

| 指令 | 功能 |
|------|------|
| /help | 帮助 |
| /win | 显示胜利者 |
| /r | 当前模式 |
| /ban <name> | 封禁玩家 |
| /kick <name> | 踢玩家 |
| /cban <color> | 按颜色封禁 |
| /ckick <color> | 按颜色踢 |
| /dump | 复制日志 |
| /endgame | 结束游戏 |
| /endmeeting | 结束会议 |
| /aur | 社交链接 |
| /color | 改颜色 |
| /reload | 重置语言 |
| /start | 开始游戏 |

---

# ⭐ 著名

- AUnlocker
- EHR (TOHE+)
- TOH
- Super New Roles
- TOHY
- MoreGamemodes
---

#链接

- 芙酱awa的B站主页:https://b23.tv/h3Hnqqf

- 這是名字awa的B站个人主页:https://b23.tv/r3e1u0x

# 📜 执照

GNU GPL v3.0

---

# ⚠️ 免责声明

本模组与 Among Us 或 Innersloth LLC 无任何关系
